...
export const meta = {
  ...
  lastEdited: '2019-08-16T19:47:20.000Z'
}

[Storybook](https://storybook.js.org/) is an open source tool for developing JavaScript UI components in isolation.

In this guide, we will cover how to set up a Svelte Storybook component library application with [Storybook CLI](https://github.com/storybookjs/storybook/tree/master/lib/cli) and how to deploy it with [ZEIT Now](/now).
...
<Snippet dark text="mkdir svelte-stories && cd svelte-stories && yarn init -y" />
...
<Snippet dark text="yarn add -D @storybook/svelte @babel/core babel-loader" />
...
By adding these scripts you are able to start a development server from inside your project directory with `yarn storybook`. You are also able to deploy the Storybook component library to [ZEIT Now](/now) with no additional configuration.
